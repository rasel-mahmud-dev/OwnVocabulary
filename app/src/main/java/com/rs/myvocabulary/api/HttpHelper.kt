...
class HttpHelper private constructor() {
    ...
    companion object {
        ...
        fun getInstance(): HttpHelper {
            return instance
                    ?: synchronized(this) { instance ?: HttpHelper().also { instance = it } }
        }
    }
    ...
    suspend fun delete(url: String): HttpResponse =
            withContext(Dispatchers.IO) {
                val request = Request.Builder().url(baseUrl + url).delete().build()
                client.newCall(request).execute().use { response ->
                    HttpResponse(response.code, response.body?.string())
                }
            }

    suspend fun patch(url: String, json: String): HttpResponse =
            withContext(Dispatchers.IO) {
                val requestBody = json.toRequestBody("application/json".toMediaTypeOrNull())
                val request = Request.Builder().url(baseUrl + url).patch(requestBody).build()
                client.newCall(request).execute().use { response ->
                    HttpResponse(response.code, response.body?.string())
                }
            }
    ...
    suspend fun put(url: String, json: String): HttpResponse =
            withContext(Dispatchers.IO) {
                val requestBody = json.toRequestBody("application/json".toMediaTypeOrNull())
                val request = Request.Builder().url(baseUrl + url).put(requestBody).build()
                client.newCall(request).execute().use { response ->
                    HttpResponse(response.code, response.body?.string())
                }
            }

    suspend fun post(url: String, json: String): HttpResponse =
            withContext(Dispatchers.IO) {
                val requestBody = json.toRequestBody("application/json".toMediaTypeOrNull())
                val request = Request.Builder().url(baseUrl + url).post(requestBody).build()
