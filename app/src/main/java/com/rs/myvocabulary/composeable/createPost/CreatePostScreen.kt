...
fun CreatePostScreen(
    ...
    // now here handler a case like if data comes from other app like
    LaunchedEffect(initialText) {
        if (initialText != null) {
            if (initialText.startsWith("<<<<< app:WebScrap >>>>>")) {
                val content =
                    initialText
                        .substringAfter("<<<<< app:WebScrap >>>>>")
                        .substringBefore("<<<<< app:WebScrap:Images >>>>>")
                        .trim()
                val imagesPart =
                    initialText
                        .substringAfter("<<<<< app:WebScrap:Images >>>>>")
                        .trim()

                textContent = content

                if (imagesPart.isNotBlank()) {
                    val images =
                        imagesPart
                            .split("\n")
